...
async function handlePlayStart() {
        ...
        create: gameServer.scorebotEvents.map((event) => {
          switch (event.type) {
            ...
            case Scorebot.EventIdentifier.PLAYER_KILLED: {
              ...
              const attacker = players.find((player) => player.name === eventKilled.attacker.name);
  ...
  mainWindow.restore();
  WindowManager.send(Constants.WindowIdentifier.Modal, { target: '/postgame', payload: match.id });
